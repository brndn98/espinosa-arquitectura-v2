...
//HANDLERS
const navContact = document.getElementById("nav-contact"); //handles the contact button on the navigation bar
const contactContainer = document.getElementById("contact-container"); //handles the contact dialog container, for displaying
const contactContent = document.getElementById("contact-content"); //handles the contact dialog content, for dynamic styling

const inputName = document.getElementById("contact-name"); //handles the input field for the name on the contact dialog
const inputMail = document.getElementById("contact-email"); //handles the input field for the e-mail on the contact dialog
const inputSubject = document.getElementById("contact-subject"); //handles the input field for the subject on the contact dialog
const inputMessage = document.getElementById("contact-message"); //handles the input field for the message on the contact dialog
const inputSubmit = document.getElementById("contact-submit"); //handles the submit button on the contact dialog
...
// /^[\w\.\-_]{1,}@[\w\.\-]{6,}/;
// /^(?=.*[@])(?=.*[.])/;
let contactChecked = false; //flag for the form validation when every field is filled
...
function closeContact() {
  //function for closing the contact dialog
...
function eraseAnswer() {
  //function for erasing the answer text from the contact dialog

  if (searchAnswer.length != 0) {
    //if there's an element, hence an answer already, then enter
...
function validateInput() {
  //function for validating every input field
  ...
  if (
    ...
    //if every field is filled, then enter
    if (validateEmail()) {
      //if the e-mail is valid, then enter
...
inputName.oninput = function() {
  //function for validating the name input field on typing
  validateInput();
};
inputMail.oninput = function() {
  //function for validating the email input field on typing
  validateEmail();
  validateInput();
};
inputSubject.oninput = function() {
  //function for validating the subject input field on typing
  validateInput();
};
inputMessage.oninput = function() {
  //function for validating the message input field on typing
  validateInput();
};

function disableInputs() {
  //function for disabling the inputs
  ...
  if (contactContainer.className == "contact-container-hidden") {
    //if the contact dialog is hidden, then enter
  ...
  if (contactChecked) {
    //if the the contact form is all filled, then enter
